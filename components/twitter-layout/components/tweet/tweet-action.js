...
export default function TweetAction({ tweet }) {
  ...
  return (
    <>
      {isConversation ? (
        <a href={tweetUrl} title="View the conversation on Twitter" target="_blank" rel="noopener noreferrer">
          <div className="icon icon-reply" />
          <span className="text">{count ? formatNumber(count) : tweet.ctaCount} people are talking about this</span>
          <div className="icon icon-chevron" />
        </a>
      ) : (
        <a href={userUrl} title={`View ${tweet.name}'s profile on Twitter`} target="_blank" rel="noopener noreferrer">
